        ...
        switch ($action) {
            ...
            case 'save_concessione_edit':
                ...
                foreach ($updates as $col => $val) {
                    ...
                    // CORREZIONE CRITICA: La logica di gestione dei parametri per i valori NULL era errata.
                    // Ora, ogni valore, che sia una stringa, un numero o NULL, viene aggiunto all'array
                    // dei parametri, garantendo che gli indici ($1, $2, ...) siano sempre corretti.
                    // Questo replica fedelmente il comportamento dell'originale.
                ...
                // Se l'update non ha modificato righe (es. record inesistente), tenta un insert.
...
function normalize_value_for_db($v, $type) {
    ...
    // CORREZIONE: La stringa 'NULL' proveniente dal JS (per i select vuoti) deve diventare un vero NULL.
    ...
    return $v;
            ...
            case 'save_concessione_edit':
                ...
                if (!$res || pg_affected_rows($res) === 0) {
                    ...
                    if (!in_array(pg_escape_identifier($conn, 'idf24'), array_map('strtolower', $insert_cols))) {
