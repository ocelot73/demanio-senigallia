...
.width-mode-narrow th[data-column*="concessionario"] { width: 250px; } /* Eccezione per colonna concessionario */
...
html.dark-theme #editModal .modal-container {
    background: rgba(30, 41, 59, 0.7); /* Dark theme glass */
    border: 1px solid rgba(51, 65, 85, 0.5);
}
#editForm { box-sizing: border-box; }
#editModal .modal-content { padding: 1.5rem; }
...
.edit-field .edit-input::placeholder { color: var(--color-text-secondary); opacity: 0.7; } /* Style for NULL placeholder */
...
/* Stili per Help Icon & Popup */
.help-dot{
  --md-surface:#F1F3FF; --md-outline:#C5CAE9; --md-ink:#0D47A1;
...
.help-pop{position:fixed;z-index:9999;min-width:260px;max-width:520px;background:var(--color-surface);border:1px solid var(--color-border);border-radius:var(--radius-lg);box-shadow:var(--shadow-lg);padding:12px 14px;font-family:var(--font-family); opacity: 0; transition: opacity 0.2s ease-in-out; max-height: calc(100vh - 20px); overflow-y: auto;}
.help-pop.open{opacity: 1;}
...
/* --- PAGINA IMPORTAZIONE --- */
.import-container .card, .import-container .card-container { background: var(--color-surface); border-radius: var(--radius-lg); box-shadow: var(--shadow-sm); padding: 2.5rem; border: 1px solid var(--color-border); max-width: 800px; margin: 2rem auto; }
